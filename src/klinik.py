...
class klinikWindow(QDialog):
    ...
    def load_data(self):
        ...
        self.tableWidget.setColumnCount(len(data[0])-1) # Kurangi 1 karena IDKlinik dihapus
        self.tableWidget.setRowCount(len(data))
        self.tableWidget.setHorizontalHeaderLabels(['ID Klinik', 'Nama Klinik', 'Alamat', 'Jam Praktek', 'Telepon'])

        self.tableWidget.takeHorizontalHeaderItem(0) # Hapus judul header kolom IDKlinik

        for row_num, row_data in enumerate(data):
            for col_num, col_data in enumerate(row_data[1:]): # Mulai dari indeks ke-1 karena IDKlinik dihapus
                self.tableWidget.setItem(row_num, col_num, QtWidgets.QTableWidgetItem(str(col_data)))

        self.tableWidget.setHorizontalHeaderLabels(['Nama Klinik', 'Alamat', 'Jam Praktek', 'Telepon']) # Atur ulang judul header kolom